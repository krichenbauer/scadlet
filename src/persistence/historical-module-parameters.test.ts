...
import { DefinitionRegistry, moduleParameterPortId } from '../editor/definitions'
...
describe('historical v3 Module parameter persistence', () => {
  it('normalizes, restores, evaluates, and reserializes the pre-Phase-4 fixture without losing signature data', async () => {
    ...
    expect(Object.keys((inputs as ModuleInputsNode).outputs)).toEqual(definition.parameters.map((parameter) => moduleParameterPortId(parameter.id)))
    expect(Object.keys((call as ModuleCallNode).inputs)).toEqual(definition.parameters.map((parameter) => moduleParameterPortId(parameter.id)))
