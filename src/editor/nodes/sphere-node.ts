...
export class SphereNode extends ClassicPreset.Node<Record<string, ClassicPreset.Socket>, { geometry: ClassicPreset.Socket }, SphereControls> implements DataflowNode {
  ...
  constructor(params: Partial<SphereParams> = {}, notify?: () => void, canRemoveInputs?: (keys: readonly string[]) => boolean) {
    super(t('node.sphere')); this.notify = notify; this.canRemoveInputs = canRemoveInputs; this.values = Object.keys(params).length === 0 && notify === undefined ? { mode: 'radius', r: 5, d: 10 } : { ...params }
  ...
  private addSize(mode: SphereSizeMode): void {
    if (this.controls.mode && !this.canRemove(this.sizeInputKeys())) { this.controls.mode.value = this.values.mode ?? 'radius'; return }
    ...
    const control = new SelectControl<SphereSizeMode>(t('control.size'), [{ value: 'radius', label: t('mode.radius') }, { value: 'diameter', label: t('mode.diameter') }], mode)
    control.onChange = (next) => { this.addSize(next); this.notify?.() }
    control.canChange = () => this.canRemove(this.sizeInputKeys())
    this.addControl('mode', control); this.addNumber(mode === 'radius' ? 'r' : 'd', mode === 'radius' ? t('control.radius') : t('control.diameter'), mode === 'radius' ? this.values.r ?? 5 : this.values.d ?? 10)
  }
  ...
  private actions(): readonly { id: string; label: string; run: () => void }[] { const result: { id: string; label: string; run: () => void }[] = []; if (!this.controls.mode) result.push({ id: 'size', label: t('action.addSize'), run: () => { this.addSize('radius'); this.notify?.() } }); if (this.values.fn === undefined) result.push({ id: 'fn', label: t('action.addFn'), run: () => { this.addNumber('fn', t('control.fn'), 30); this.notify?.() } }); return result }
