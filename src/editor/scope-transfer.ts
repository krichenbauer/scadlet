...
export type ScopeTransferProblem = 'protected' | 'module-call' | 'connection' | 'function-incompatible' | 'function-recursion'
...
export function scopeTransferProblem(
  ...
  for (const nodeId of moved) {
    ...
    // Module Calls remain Main-only. Function Calls may enter a Function
    // scope, but not a Module scope; recursion is preflighted by editor.ts
    // against the proposed final ownership after this structural check.
    if (targetScope !== null && node instanceof ModuleCallNode) return 'module-call'
    if (targetScope !== null && node instanceof FunctionCallNode && !targetIsFunction) return 'module-call'
