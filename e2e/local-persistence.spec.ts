...
test('creates, displays, protects, and restores a Module definition', async ({ page }) => {
  ...
  await expect(frame).toContainText('wheel')
...
test('creates, renders, inspects, and restores a parameterless Module Call', async ({ page }) => {
  ...
  await call.locator('.node-title').dblclick()
  await expect(page.locator('scadlet-app .scad-output')).toContainText('module wheel()', { timeout: 15_000 })
...
test('builds a terminating self-recursive Function visibly and renders it after autosave reload', async ({ page }) => {
  ...
  const one = page.locator('node-editor .node').filter({ has: page.locator('.node-title[aria-label="Number Name"]') })
  ...
  // These nodes are deliberately spread wide inside the definition frame;
  // bring them fully into view before interacting so the click's own real
  // pointer coordinate never lands under an adjacent pane.
  ...
  await restoredInputs.getByRole('button', { name: 'Rename n', exact: true }).click()
  await restoredInputs.locator('.node-interface-rename-input').fill('value')
  await restoredInputs.locator('.node-interface-rename-input').press('Enter')
  await expect(page.locator('node-editor svg.connection[data-real-connection="true"]')).toHaveCount(11)
...
test('restores, evaluates, renders, autosaves, and reloads direct and mutually recursive Functions', async ({ page }) => {
  ...
  // This fixture's nodes are spread across a wide area; bring them fully
  // into view before the next direct node interaction.
  await page.getByRole('button', { name: 'Fit graph', exact: true }).click()
  await page.locator('node-editor .node[data-node-id="main-even"] .node-title').dblclick()
...
test('preselects palette/header operations with readable controls and safely changes atan2 dynamic inputs', async ({ page }) => {
  ...
  await Promise.resolve()
  await vectorCube.locator('.node-add-summary').click()
  ...
  const numbers = page.locator('node-editor .node').filter({ has: page.locator('.node-title[aria-label^="Number"]') })
...
test('restores, renders, edits, autosaves, reloads, and safely deletes recursive Modules', async ({ page }) => {
  ...
  await stackInputs.getByRole('button', { name: 'Rename n', exact: true }).click()
  await stackInputs.locator('.node-interface-rename-input').fill('levels')
  await stackInputs.locator('.node-interface-rename-input').press('Enter')
  await stackInputs.getByRole('button', { name: 'Rename Layer', exact: true }).click()
  await stackInputs.locator('.node-interface-rename-input').fill('Slice')
  await stackInputs.locator('.node-interface-rename-input').press('Enter')
  ...
  await page.getByRole('button', { name: 'Render', exact: true }).click()
  await expect(source).toContainText('module stack_layers(levels = 1)', { timeout: 15_000 })
  ...
  expect(saved.project.version).toBe(8)
...
test('adds a typed Module parameter and materializes matching border-anchored Call input', async ({ page }) => {
  ...
  await dialog.getByRole('button', { name: 'Create', exact: true }).click()
  const frame = page.locator('node-editor .definition-frame').filter({ hasText: 'ball' })
  const definitionId = await frame.getAttribute('data-definition-id')
  if (!definitionId) throw new Error('Expected Module definition id')
  const inputs = page.locator('node-editor .node').filter({ has: page.locator('.node-title', { hasText: 'Inputs' }) })
  await inputs.locator('.node-add-summary').click()
...
test('two Geometry inputs with the second wired to Output render exactly one Geometry socket per built-in node and generate children(1);', async ({ page }) => {
  ...
  await dialog.getByRole('button', { name: 'Create', exact: true }).click()
  const frame = page.locator('node-editor .definition-frame').filter({ hasText: 'test123' })
  const definitionId = await frame.getAttribute('data-definition-id')
  if (!definitionId) throw new Error('Expected Module definition id')
  const inputs = page.locator('node-editor .node').filter({ has: page.locator('.node-title', { hasText: 'Inputs' }) })
  const output = page.locator('node-editor .node').filter({ has: page.locator('.node-title', { hasText: 'Output' }) })
  ...
  const geometryInput = inputs.getByRole('group', { name: 'Geometry input', exact: true })
  await expect(geometryInput).toBeVisible()
  await expect(inputs.locator('.node-parameter-popover')).toHaveCount(0)
  await geometryInput.getByLabel('Geometry input name').fill('Fnord')
  await geometryInput.getByRole('button', { name: 'Add', exact: true }).click()
  await expect(inputs.locator('.node-socket[aria-label="Fnord"]')).toHaveCount(1)
...
test('renames and deletes a Module through its sidebar actions', async ({ page }) => {
  ...
  await expect(page.locator('node-editor .definition-frame')).toContainText('rim')
  await waitForAutosave(page)
  await page.reload()
...
test('deletes a connected Module parameter only after confirmation and persists every projected removal', async ({ page }) => {
  ...
  // Cancelling is a true no-op: the row stays untouched so the user can
  // reconsider, and none of its exact connections have been touched.
  await page.evaluate(() => {
  ...
  await inputs.getByRole('button', { name: 'Remove radius', exact: true }).click()
  expect(await page.locator('html').getAttribute('data-last-confirmation')).toContain('disconnect 3 connection(s)')
  await expect(inputs.getByRole('button', { name: 'Remove radius', exact: true })).toBeVisible()
...
test('shows a localized error and preserves state when Module parameter deletion cannot start', async ({ page }) => {
  await waitForLocalLibrary(page)
  const project = structuredClone(HISTORICAL_MODULE_PARAMETERS)
  project.editor.viewport = { x: 0, y: 0, zoom: 1 }
  project.graph.nodes[0].position = { x: 40, y: 420 }
...
test('Cube Size add menu exposes one selected representation at a time', async ({ page }) => {
  ...
  await node.getByRole('button', { name: 'Remove Size', exact: true }).click()
  await node.locator('.node-add-summary').click()
  await node.getByText('Size', { exact: true }).click()
  await node.getByRole('button', { name: 'XYZ', exact: true }).click()
  ...
  await node.locator('.node-add-summary').click()
  await node.getByText('Size', { exact: true }).click()
  await node.getByRole('button', { name: 'Vector', exact: true }).click()
...
test('vector transforms expose one representation and Center has a Boolean row', async ({ page }) => {
  ...
  await translate.getByRole('button', { name: 'Remove XYZ', exact: true }).click()
  await translate.locator('.node-add-summary').click()
  await translate.getByRole('button', { name: 'Vector', exact: true }).click()
  ...
  await translate.getByRole('button', { name: 'Remove Vector', exact: true }).click()
  await translate.locator('.node-add-summary').click()
  await translate.getByRole('button', { name: 'XYZ', exact: true }).click()
...
test('typed value nodes remain compact and a Number drives Cube Size', async ({ page }) => {
  ...
  const cube = page.locator('node-editor .node').filter({ has: page.locator('.node-title', { hasText: 'Cube' }) })
  const valueRow = number.locator('.node-param-row[data-param-key="value"]')
  await expect(valueRow.locator('input[type="number"]')).toBeVisible()
  await expect(valueRow).toHaveAttribute('data-value-source', 'fallback')
  await expect(valueRow).toHaveAttribute('aria-label', 'Value: Fallback value')
  await expect(number.locator('.node-title')).toHaveText('Number')
  await expect(number.locator('.node-collapse')).toHaveCount(0)
  await expect(valueRow.locator('.node-param-label')).toHaveText('Value')
  ...
  await valueRow.locator('input[type="number"]').fill('20')
  await Promise.resolve()
  await cube.locator('.node-add-summary').click()
  await cube.getByText('Size', { exact: true }).click()
  await cube.getByRole('button', { name: 'Scalar', exact: true }).click()
...
test('rejects a visible node dataflow cycle without changing the valid graph, then renders through bundled OpenSCAD-WASM', async ({ page }) => {
  ...
  await dropPaletteNode(page, 'cube', { x: canvas.x + 120, y: canvas.y + 350 })
  ...
  await cube.getByText('Size', { exact: true }).click()
  await cube.getByRole('button', { name: 'Scalar', exact: true }).click()
  const cubeCollapse = cube.getByRole('button', { name: 'Collapse node' })
  await expect(cubeCollapse).toHaveAttribute('aria-expanded', 'true')
  await expect(cubeCollapse.locator('svg[aria-hidden="true"]')).toHaveCount(1)
...
test('creates a Compare with direct fallbacks, feeds Geometry If, and restores it through autosave', async ({ page }) => {
  ...
  const canvas = await page.locator('node-editor').boundingBox()
  if (!canvas) throw new Error('Expected node-editor canvas')
  await dropPaletteNode(page, 'compare', { x: canvas.x + 120, y: canvas.y + 130 })
  await dropPaletteNode(page, 'cube', { x: canvas.x + 410, y: canvas.y + 80 })
  await dropPaletteNode(page, 'sphere', { x: canvas.x + 410, y: canvas.y + 310 })
  await dropPaletteNode(page, 'if', { x: canvas.x + 690, y: canvas.y + 190 })
  ...
  await expect(compare.locator('[data-param-key="b"] input')).toHaveValue('10')
...
test('source names persist and value Inspect evaluates Arithmetic headlessly through OpenSCAD', async ({ page }) => {
  await waitForLocalLibrary(page)
  await dropPaletteNode(page, 'number')
  // The aria-label (derived from the node TYPE, not its custom title) stays
  // constant whether the title is currently static text or the rename
  // input, unlike a text-content filter which would stop matching once the
  // title becomes an (empty-text-content) <input> mid-rename.
  const number = page.locator('node-editor .node').filter({ has: page.locator('.node-title[aria-label="Number Name"]') })
  // The title is normal text by default; Rename lives in the header More
  // menu (node-style.md "Value nodes").
  ...
  await waitForAutosave(page)
  await page.reload()
  const restoredNumber = page.locator('node-editor .node').filter({ has: page.locator('.node-title', { hasText: 'Width' }) })
  await expect(restoredNumber.locator('.node-title')).toHaveText('Width')
  // The title is now ordinary node surface text (not an editable field), so
  // a double-click there also starts a one-shot value Inspect, the same as
  // anywhere else on the compact node body.
  await restoredNumber.locator('.node-title').dblclick()
  ...
  await restoredNumber.locator('.node-param-row[data-param-key="value"] .node-param-label').dblclick()
  await expect(restoredNumber.locator('.node-inspect-value')).toHaveText('= 10', { timeout: 15_000 })
...
test('Boolean and Vector3 use renameable source titles, typed input rows, and no redundant output labels', async ({ page }) => {
  await waitForLocalLibrary(page)
  const canvas = await page.locator('node-editor').boundingBox()
  if (!canvas) throw new Error('Expected node-editor canvas')
  // Distinct drop points - dropPaletteNode without a position always targets
  // the canvas center, which would otherwise stack both nodes exactly on top
  ...
  await boolean.locator('input.node-title').fill('Centered')
  await boolean.locator('input.node-title').press('Enter')
  await expect(boolean.locator('.node-title')).toHaveText('Centered')
  await expect(boolean.locator('.node-param-row[data-param-key="value"] .node-param-label')).toHaveText('Value')
  await expect(boolean.locator('.node-collapse')).toHaveCount(0)
  await expect(boolean.locator('.node-port--output .node-port-label')).toHaveCount(0)
  await expect(vector.locator('.node-title')).toHaveText('Vector3')
  await vector.locator('.node-more-summary').click()
  await vector.getByRole('menuitem', { name: 'Rename' }).click()
  await vector.locator('input.node-title').fill('Position')
  await vector.locator('input.node-title').press('Enter')
  await expect(vector.locator('.node-title')).toHaveText('Position')
  await Promise.resolve()
  await expect(vector.locator('[data-param-key="x"], [data-param-key="y"], [data-param-key="z"]')).toHaveCount(3)
  await expect(vector.locator('[data-param-key="value"] .node-param-label')).toHaveText('Value')
  await expect(vector.locator('[data-param-key="value"] .node-socket')).toHaveAttribute('data-socket-type', 'vector3')
...
test('explicitly collapsed controls stay hidden during ordinary and wire pointer movement', async ({ page }) => {
  ...
  await dropPaletteNode(page, 'number')
  await dropPaletteNode(page, 'cube')
  const number = page.locator('node-editor .node[data-renameable="true"]')
  const cube = page.locator('node-editor .node').filter({ has: page.locator('.node-title', { hasText: 'Cube' }) })
  await cube.locator('.node-add-summary').click()
  await cube.getByText('Size', { exact: true }).click()
  await cube.getByRole('button', { name: 'Scalar', exact: true }).click()
...
test('drag and click wire gestures never reveal collapsed rows and explicit expansion preserves click wiring', async ({ page }) => {
  ...
  // Separate this collapsed Cube's own header/collapse control from the
  // Number source's fixed drop point (both otherwise default to the same
  // canvas center), so the pending-wire hover below cannot coincidentally
  // land on an unrelated interactive control.
  await moveNode(cube, 220, 0)
  const cubeHeader = await cube.locator('.node-header').boundingBox()
...
test('connected compact rows preserve canonical order when expanded', async ({ page }) => {
  ...
  const numberSources = page.locator('node-editor .node').filter({ has: page.locator('.node-title[aria-label="Number Name"]') })
  let numberIndex = 0
  ...
  await dropPaletteNode(page, 'translate')
  const translate = page.locator('node-editor .node').filter({ has: page.locator('.node-title', { hasText: 'Translate' }) })
  await moveNode(translate, -220, -220)
...
test('Vector3 connected rows retain canonical order', async ({ page }) => {
  ...
  const vector = page.locator('node-editor .node[data-renameable="true"]').filter({ has: page.locator('[data-param-key="x"]') })
  ...
  await expect.poll(() => visibleRowKeys(vector)).toEqual(['value', 'x', 'y', 'z'])
...
test('Cube XYZ connected rows retain canonical order', async ({ page }) => {
  ...
  await expect(cube.locator('.node-add-menu')).toHaveCount(1)
})
...
test('imports the static legacy v5 arithmetic fixture as canonical v8 nodes', async ({ page }) => {
  await waitForLocalLibrary(page)
  const chooserPromise = page.waitForEvent('filechooser')
  await (await fileAction(page, 'Open')).click()
  const chooser = await chooserPromise
